...
export class ClothSolverWebGPU {
  ...
  private generateAttachmentConstraints(attachIndices: number[], positions: Float32Array): void {
    ...
    for (let i = 0; i < attachIndices.length; i++) {
      ...
      if (particleIdx >= numParticles) {
        console.warn(`Attachment index ${particleIdx} exceeds particle count ${numParticles}`);
        continue;
      }
  ...
  private async initPipelines(): Promise<void> {
    // Load shaders
    const predictShader = await fetch('/src/shaders/predict_positions.wgsl').then((r) => r.text());
    const finalizeShader = await fetch('/src/shaders/finalize_velocities.wgsl').then((r) => r.text());
    const stretchShader = await fetch('/src/shaders/solve_stretch.wgsl').then((r) => r.text());
    const attachmentShader = await fetch('/src/shaders/solve_attachment.wgsl').then((r) => r.text());
    const applyDeltasShader = await fetch('/src/shaders/apply_deltas.wgsl').then((r) => r.text());
  ...
  private createBindGroups(): void {
    ...
    if (this.solveAttachmentPipeline && this.attachmentConstraintsBuffer && this.slotPositionsBuffer) {
  ...
  async simulate(deltaTime: number): Promise<void> {
    ...
    const uniformData = new Float32Array([deltaTime, this.params.gravity, this.params.particleDiameter, this.params.numParticles]);
  ...
  async getPositions(): Promise<Float32Array> {
    const numParticles = (this.config.resolution + 1) * (this.config.resolution + 1);
    const bufferData = await this.bufferManager.readBuffer(this.positionBuffer, numParticles * 12);
